  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="mb-6 flex gap-2">
        <button onClick={() => setTab("users")} className={`rounded-lg px-4 py-2 text-sm font-medium ${tab === "users" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"}`}>Users</button>
        <button onClick={() => setTab("audit")} className={`rounded-lg px-4 py-2 text-sm font-medium ${tab === "audit" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"}`}>Audit Log</button>
      </div>
      {loading ? (
