    ...
    <header className="sticky top-0 z-50 border-b border-gray-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
        ...
        <nav className="hidden items-center gap-6 md:flex">
          <a href={`/${isAr ? "ar" : ""}#features`} className="text-sm text-gray-600 hover:text-gray-900">Features</a>
          <a href={`/${isAr ? "ar" : ""}#compliance`} className="text-sm text-gray-600 hover:text-gray-900">Compliance</a>
          {user ? (
            <>
              <a href={user.role === "admin" ? "/admin" : "/patient"} className="text-sm font-medium text-blue-600 hover:text-blue-700">Dashboard</a>
              <button onClick={onLogout} className="rounded-lg border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50">Sign Out</button>
            </>
          ) : (
            <>
